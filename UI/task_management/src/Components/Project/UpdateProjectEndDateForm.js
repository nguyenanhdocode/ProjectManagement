...
export default function UpdateProjectEndDateForm({
    ...
    async function onSubmit(data) {
        ...
        try {
            setProcessing(true);
            const url = endpoints['updateProjectEndDate'].replace('{0}', project['id']);
    ...
    async function onEndDateChange(value) {
        ...
        try {
            let url = endpoints['checkBeforeUpdateProjectEndDate'].replace('{0}', projectId);
        ...
        <Dialog open={true} PaperProps={{ component: 'form' }}
            ...
            <DialogContent>
                ...
                <CustomDateTimePicker control={control} name='endDate'
                    error={Boolean(errors?.beginDate)} label={t('common.endDate')}
                    helperText={errors?.beginDate?.message} variant='filled'
                    required={true} labelShink={true} customOnClose={onEndDateChange}>
