...
export default function CreateSubtaskForm({ onCancel, onSuccess, taskId }) {
        ...
        beginDate: yup.date()
            ...
            .test({
                ...
                test: function (value) {
                    console.log(value)
                    console.log(task)
                    // console.log(new Date(task['beginDate']))
                    if (value < convertUTCDateToLocalDate(new Date(task['beginDate'])))
                        return this.createError({message: t('task.beginDate.mustLater')
                            .replace('{0}', formatDateTime(convertUTCDateToLocalDate(new Date(task['beginDate']))))})
                    return true;
                }
            }),
        endDate: yup.date()
            ...
            .test({
                ...
                message: t('task.endDate.mustBefore')
                    .replace('{0}', formatDateTime(convertUTCDateToLocalDate(new Date(task['endDate'])))),
                test: function (value) {
    ...
    async function onSubmit(data) {
        setProcessing(true);
        try {
            const res = await bearerRequest.post(endpoints['createSubtask'].replace('{0}', taskId), data);
